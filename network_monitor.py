...
import re
import shlex
from utils.subprocess_safe import safe_run, safe_popen
...
def block_ip(ip, reason=None, country=None, port=None):
    ...
    try:
        # ip was already validated above, so it is safe to build netsh args directly
        rule_name = "Block_" + ip.replace(":", "_")
        result = safe_run(
...
def unblock_ip(ip):
    ...
    try:
        ...
        # ip was already validated above, so build netsh args directly
        rule_arg = "name=Block_" + ip.replace(":", "_")
        result = safe_run(
