...
def install_startup():
    ...
    if not os.path.isfile(exe_path) or not os.path.isabs(exe_path):
        print("Cannot install startup: executable path could not be resolved.")
        return False
    ...
    try:
        tr_value = subprocess.list2cmdline([exe_path])
        subprocess.run(
            ['schtasks', '/create', '/tn', 'AntivirusYARAServerStartup',
             '/tr', tr_value, '/sc', 'ONLOGON', '/f'],
            check=True, capture_output=True, text=True,
            creationflags=CREATE_NO_WINDOW
        )
...
def get_encryption_key():
    """Derive quarantine encryption key from env + machine-specific salt."""
    # Use a combination of machine-specific values as salt
    salt = socket.gethostname().encode() + b'antivirus_quarantine_salt'
    # In production this passphrase must be set in the environment
    password = os.environ.get('QUARANTINE_PASSWORD', '').encode()
    if not password:
        raise RuntimeError('QUARANTINE_PASSWORD environment variable is not set')
...
def quarantine_yara_matches():
    ...
    for entry in list(latest_yara_suspicious):
        ...
        if not path or not rules or path in seen:
            continue
        # Require rule names that CONTAIN persistence/ransomware
        # and let safe_quarantine refuse protected system locations.
        is_ransomware = any('ransomware' in str(m).lower() for m in rules)
        is_persistence = any('persistence' in str(m).lower() for m in rules)
