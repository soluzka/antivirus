        ...
        if response.status_code == 200:
            try:
                ...
                if result.get('query_status') == 'ok':
                    ...
                    # Read existing signatures first (from backup file since we just renamed it)
                    existing_signatures = set()
                    if os.path.exists(backup_file):
                        with open(get_resource_path(backup_file), 'r', encoding='utf-8') as f:
                            for line in f:
...
def update_virus_definitions():
    ...
    Updates Windows Defender on Windows, ClamAV on macOS/Linux, plus
    Malware Bazaar signatures and YARA rules on all platforms.
    """
    ...
    if system == "Windows":
        system_av_success = update_windows_defender_definitions()
    else:
        system_av_success = update_clamav_definitions()

    malware_bazaar_success = update_malware_bazaar_signatures()
    yara_rules_success = update_yara_rules()

    # Log overall result
    if system_av_success and malware_bazaar_success and yara_rules_success:
        logging.info("All virus definitions updated successfully")
    else:
        if not system_av_success:
            if system == "Windows":
                logging.warning("Windows Defender definitions update failed")
            else:
                logging.warning("ClamAV definitions update failed or ClamAV not installed")
    ...
    return system_av_success or malware_bazaar_success or yara_rules_success


def update_clamav_definitions():
    """
    Update ClamAV virus definitions on macOS/Linux/ChromeOS.
    Returns True if successful, False if ClamAV not installed or update failed.
    """
    import platform as pf
    if pf.system() == "Windows":
        return False  # Use Windows Defender on Windows

    freshclam = shutil.which('freshclam')
    if not freshclam:
        logging.debug("ClamAV (freshclam) not installed — skipping definition update")
        return False
    ...
    try:
        ...
        if result.returncode == 0:
            logging.info("ClamAV definitions updated successfully")
            return True
        else:
            logging.error(f"ClamAV update error: {result.stderr}")
            return False
    except subprocess.TimeoutExpired:
        logging.error("Timeout while updating ClamAV definitions")
        return False
    except Exception as e:
        logging.error(f"Error updating ClamAV definitions: {str(e)}")
        return False
