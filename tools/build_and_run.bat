...
taskkill /f /im IsolationBytesLogin.exe >nul 2>&1
...
>"%LOG%" 2>&1 (
    ...
    if exist "%BUILD%\AntivirusServerLogin.exe" (
        copy /y "%BUILD%\AntivirusServerLogin.exe" "%DIST%\IsolationBytesLogin.exe"
        echo copied to %DIST%\IsolationBytesLogin.exe
    ) else (
